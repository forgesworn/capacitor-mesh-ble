...
## Release Notes
- Conventional commit-style prefixes: `feat:`, `fix:`, `docs:`, `chore:`, `refactor:`. Do not include `Co-Authored-By` lines in commits.
...
`package.json`'s version can run ahead of the last tag when changes land under `CHANGELOG.md`'s `[Unreleased]` heading before being tagged and released.
